...
export function Dashboard() {
  ...
  const schedules: Schedule[] = [
    { id: "1", time: "09:00", title: "팀 회의", type: "personal" },
        ...
        <section className="space-y-6">
            ...
            <div className="space-y-3">
              {schedules.map((schedule) => (
                <div key={schedule.id} className="flex items-start gap-3 lg:gap-4">
                  ...
                  <div className={`flex-1 rounded-2xl p-3 lg:p-4 ${schedule.type === "child" ? "bg-[#B3D9FF]" : "bg-[#FFB3D9]/20"}`}>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-md lg:p-8">
            ...
            <div className="grid gap-2 xl:grid-cols-2">
              {todos.map((todo) => (
                <div key={todo.id} className="flex items-center gap-3 rounded-2xl p-3 transition-colors hover:bg-gray-50">
                  <button onClick={() => toggleTodo(todo.id)} className="flex-shrink-0">
                    {todo.completed ? (
                      <CheckCircle2 className="h-5 w-5 text-[#FFB3D9]" />
                    ) : (
                  ...
                  <Star className={`h-4 w-4 ${getPriorityColor(todo.priority)} ${todo.priority === "high" ? "fill-current" : ""}`} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-3xl bg-white p-6 shadow-md lg:p-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-2xl bg-[#FAFAFA] p-4">
                <p className="mb-1 text-xs text-gray-500">총지출</p>
            ...
            <div className="space-y-3">
              <div className="rounded-2xl bg-[#B3D9FF]/30 p-4">
                ...
                <ul className="space-y-1 text-sm">
                  <li className="flex items-center gap-2"><Circle className="h-3 w-3 text-[#B3D9FF]" /><span>운동화 사주기</span></li>
                  <li className="flex items-center gap-2"><Circle className="h-3 w-3 text-[#B3D9FF]" /><span>주말 영화 보러 가기</span></li>
                </ul>
              </div>
              <div className="rounded-2xl bg-[#B3FFCC]/30 p-4">
                <p className="mb-2">둘째 민서</p>
                <ul className="space-y-1 text-sm">
                  <li className="flex items-center gap-2"><Circle className="h-3 w-3 text-[#B3FFCC]" /><span>미술 재료 준비</span></li>
                  <li className="flex items-center gap-2"><Circle className="h-3 w-3 text-[#B3FFCC]" /><span>친구 생일 선물</span></li>
                </ul>
              </div>
              <div className="rounded-2xl bg-[#FFB3D9]/20 p-4">
                <p className="mb-2">엄마</p>
                <ul className="space-y-1 text-sm">
                  <li className="flex items-center gap-2"><Circle className="h-3 w-3 text-[#FFB3D9]" /><span>부가 수업 등록</span></li>
                  <li className="flex items-center gap-2"><Circle className="h-3 w-3 text-[#FFB3D9]" /><span>책 읽기</span></li>
                </ul>
              </div>
            </div>
          </div>
